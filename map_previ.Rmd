...
palette_highquant <- c("#fc8d59","#e34a33","#b30000")
palette_lowquant <- c("#74a9cf","#2b8cbe","#045a8d")
...
nom_prev <- paste0(indic[ind],b_index,"psprob_",kantile, ".RData")
...
matrix4shp[,4] <- unlist(lat)
matrix4shp[,5] <- unlist(lon)
...
colnames(df_shp) <- c("first_quantile", "last_quantile", "middle_quantile", "lat","lon")
...
ggmap(back_map) +
  geom_sf(data = shp_confprev, aes(color = last_quantile), size=1.65, shape = 15) +
  scale_color_gradientn(colours=palette_highquant, limits = prob_limits, na.value = "transparent", guide = guide_colorbar(barwidth = 10, barheight = 0.5, nbin = 5, title = "last_quantile", order = 3))+
new_scale("color")+
  geom_sf(data = shp_confprev, aes(color = first_quantile), size=1.65, shape = 15) + 
  scale_color_gradientn(colours=palette_lowquant, limits = prob_limits, na.value = "transparent", guide = guide_colorbar(barwidth = 10, barheight = 0.5, nbin = 5, title = "first_quantile", order = 1)) +
new_scale("color")+
geom_sf(data = shp_confprev, aes(color = middle_quantile), size=1.65, shape = 15) +
  scale_color_gradientn(colours=white_midquant, na.value = "transparent", guide = guide_colorbar(barwidth = 1.5, barheight = 0.5, label = FALSE, frame.colour = "black", title = "mid_quantile", order = 2))+
theme(legend.direction = "horizontal",
        legend.box = "vertical",   legend.position = "right",
      plot.title = element_text(size = 12, hjust = 0, vjust = 1.5)) + #
  labs(title = paste0(indic[ind]," ", select_year," season ",kantile," forecast "))
