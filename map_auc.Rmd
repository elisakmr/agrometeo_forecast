...
score_boot <- array(NA, dim=c(ngrids, 2, nboot)) 
  ...
  for (b in 1:nboot)  { 
    ...
    nom_prev <- paste0(indic[ind], b, "_", kantile, "aucgrid.RData")
...
for (i in 1:ngrids)  { 
  ...
  wmsess <- wilcox.test(score_boot[i,quant,], mu = 0.5, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0.5" & "alternative = greater", alternative being h1
...
ggmap(back_map) +
geom_sf(data = shp_grid, aes(color = auc), size=1.65, shape = 15) + 
  scale_color_gradientn(colours=score_palette, limits = auc_limits, guide = guide_colorbar(barwidth = 10, barheight = 0.25, nbin = 5)) +
new_scale("color")+
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(), legend.text=element_text(size=6),  
  ...
  labs(title = paste0(indic[ind]," AUC ", extremes[quant], " ", kantile))
