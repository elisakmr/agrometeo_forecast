...
for (m in 1:ndimT){
  ...
  maprev [[m]]<- tm_basemap(leaflet::providers$Esri.WorldTopoMap)+ 
    qtm(htshp_score[[m]], fill = "highter_score", fill.legend.show = TRUE,
     attr.outside = TRUE,borders = NULL, fill.palette = "YlOrRd")+ 
 tm_shape(htshp_sign[[m]])+
   tm_symbols(size ="highter_significance", col = "yellow", legend.size.show = FALSE, scale = 0.1)+
