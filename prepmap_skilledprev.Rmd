...
list_match <- lapply(rand_year, function(x) grep(select_y, x))

y_index <- list_match[[1]] # year index within bootstrap iteration
...
for (i in 1:ngrids) {
  if (prob_ps[i,y_index[1],select_t,1] < prob_ps[i,y_index[1],select_t,2]) # i.e. first/lower tercile most likely to happen
  {matrix4shp[i,1] <- 3
