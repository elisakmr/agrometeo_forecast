---
title: "Mapping skilled probabilistic prevision"
output: html_document
---

```{r}
dir_data <- "D:/HOME/ekamir/scoring_medscope/data/season_trial2" 
dir_plot <- "D:/HOME/ekamir/scoring_medscope/figure" 
library(maps)
library(maptools)
library(rasterVis)
library(ggplot2)
library(latticeExtra)
library(sp)
library(raster)
library(rgdal)
library(tmap)
library(sf)
library(ggmap)

```

## Parameter setting 

we have to do a trick to find out the first bootstrapped iteration and the run index whithin this iteration matching the year we are interested in (the random sampling doesn't respect chronology)

```{r}

moi <- c("May","June","July","August","September","October","November")
select_var <- 1 # indicator
select_t <- 3 # month
select_y <- 5 # year

# bootstrap iteration index of the year selected
load(file = file.path(dir_data, "extract", "years.RData")) # list of years in each bootstrap iteration

list_match <- lapply(rand_year, function(x) grep(select_y, x)) # list of matchs between list of years randomly generated for the bootstrap and the year we are interested in

y_index <- list_match[[1]] # year index within the bootstrap iteration
b_index <- 1 # bootstrap iteration index
for (b in 1:999){ # pick the first iteration and first index matching the year we are looking for
  if (length(list_match[[b]])<1 & length(y_index)<1) {y_index <- list_match[[b+1]]; b_index <- b+1}
  else {y_index <- y_index; b_index <- b_index}
}

```

## Preparing the dataframe containing previsions and associated skills

We create a conversion-to-shapefile-ready dataframe containing the most likely tercile and the values of Wilcoxon test. For that test, 1 is set (0 instead) when the wilcoxon test outcome is positive + the AUC mean is above 0.5 (which is performed by random guessing prevision).

```{r}

# loading dataframe of prevision with associated skills and coordinates

nom <- paste0(indic[select_var],"_skills.RData")
load(file = file.path(dir_data, "score", nom))

nom_ps <- paste0(indic[ind],b_index,"_prob_prev.RData")
load(file = file.path(dir_data, "extract", nom_ps))

load(file = file.path(dir_data, "extract", "lon.RData"))
load(file = file.path(dir_data, "extract", "lat.RData"))

# creating a table containing most likely tercile and its associated wilcoxon outcome, with both coordinates (shape ready for shapefile conversion)
 
matrix4shp <- matrix(1, nrow = ngrids, ncol = 4)
  
for (i in 1:ngrids) {
  if (prob_ps[i,y_index[1],select_t,1] > prob_ps[i,y_index[1],select_t,2]) # i.e. upper/lower tercile most likely to happen
  {matrix4shp[i,1] <- 3
    if (wilcox_df[i,select_t,1,2]<0.5){ # conditioning on AUC value which has to be above 0.5 (random guess)
     matrix4shp[i,2] <- wilcox_df[i,select_t,2,2] # extracting wilcoxon  test outcome (1/0) associated to the most likely tercile
    }
  }
  else if (wilcox_df[i,select_t,1,1]<0.5){
    matrix4shp[i,2] <- wilcox_df[i,select_t,2,1]
  }
}
matrix4shp[,3] <- unlist(vect_lat)
matrix4shp[,4] <- unlist(vect_lon)

```


## Converting the dataframe into shapefiles

We create two polygon shapefiles: one containing most likely tercile forecasted; one containing wilcoxon test outcomes. 

```{r}

# we load the dataframe previously built
df_shp <- data.frame(matrix4shp)
colnames(df_shp) <- c("tercile", "wilcoxon","lat","lon")
coordinates(df_shp) <- ~lon+lat
proj4string(df_shp) <- CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
sp_prev <- st_as_sf(df_shp)
ras_prev <- raster(sp_prev)
ext_prev <- extent(ras_prev)
mask.raster <- raster(resolution=c(0.1,0.1), crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0", ## !!!!!!!!!!!
                      ext = ext_prev)
ras_prev_complete <- rasterize(sp_prev, mask.raster)

# we build the shapefile of most likely tercile 
sp_tercile <- rasterToPolygons(ras_prev_complete$tercile)
shp_tercile <- st_as_sf(sp_tercile)
plot(shp_tercile)
titre_terc <- paste0(indic[select_var], "_likelytercile_", moi[select_t],an[select_y],".shp")
st_write(shp_tercile, titre_terc, delete_dsn=TRUE)

# we build the shapefile of most likely tercile 
sp_wilcox <- rasterToPolygons(ras_prev_complete$wilcoxon)
shp_wilcox <- st_as_sf(sp_wilcox)
plot(shp_wilcox)
titre_wil <- paste0(indic[select_var], "_wilcox_", moi[select_t],an[select_y],".shp")
st_write(shp_wilcox, titre_wil, delete_dsn=TRUE)
```


