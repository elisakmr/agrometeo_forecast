...
for (m in 1:ndimT){
  ...
  maprev [[m]]<-qtm(shp_tercile[[m]], fill = "tercile", fill.legend.show = FALSE,
    attr.outside = TRUE, fill.palette = c("darkslategray2","dodgerblue3"), title = titre, title.position = c("left","top"), fill.style="fixed",fill.breaks=c(1,2,3),borders = NULL)+
tm_shape(shp_wilcox[[m]])+
  tm_symbols(size ="wilcoxon", col = "black", legend.size.show = FALSE, scale = 0.01)+
#tm_grid(y=c(43.487, 43.787), x=c(4,03,4.5,5),  labels.inside.frame = TRUE, alpha = 0, labels.rot = c(0,90))+
tm_scale_bar()+
tm_add_legend(type = "fill", labels = c("Upper tercile","Lower tercile"), col = c("dodgerblue3","darkslategray2"), alpha = 0.5)
...
We display an interactive map for on specific month to be selected beforehand

```{r}

### Parameter setting
t <- 3 # month of interest
titrei <- paste0(ind_eng[ind]," ","for"," ", moi[t], " ", select_year)
...
tmap_mode('view')
  ...
  map2 <- qtm(shp_tercile, fill = "tercile",
                 fill.style="fixed",fill.breaks=c(1,2,3), fill.palette = c("darkslategray2", "dodgerblue3"), borders = NULL, fill.alpha=0, fill.legend.show = FALSE, title = titrei, main.title.position = c(""))+  
  ...
  qtm(shp_tercile[[m]], fill = "tercile",
                 fill.style="fixed",fill.breaks=c(1,2,3), fill.palette = c("darkslategray2", "dodgerblue3"), borders = NULL, fill.alpha=0, fill.legend.show = FALSE, title = titre, main.title.position = c(""))+  
  tm_shape(shp_tercile[[m]])+
  tm_fill(col ="tercile", alpha=0.6, palette = c("darkslategray2", "dodgerblue3"), legend.show = FALSE)+
     tm_shape(shp_wilcox[[m]])+
        tm_symbols(size ="wilcoxon", col = "red", alpha=1, scale = 0.3)+
  tm_scale_bar()+
