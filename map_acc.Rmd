...
matrix4shp[,1] <- apply(score_ps[,2,], 1, mean, na.rm = TRUE) #  mean of bootstrapped MSESS  

    # Wilcoxon test

for (i in 1:ngrids)  { 
    
  wmsess <- wilcox.test(score_ps[i,2,], mu = 0.3, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0.3" & "alternative = greater", alternative being h1
...
ggmap(back_map) +
geom_sf(data = shp_grid, aes(color = acc), size=1.65, shape = 15) + 
  scale_color_gradientn(colours=score_palette, limits = acc_limits, guide = guide_colorbar(barwidth = 10, barheight = 0.5, nbin = 5)) +
new_scale("color")+
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(),  
  ...
  labs(title = paste0(indic[ind]," ACC "))
