...
for (ind in 6:8){
  ...
  ## creating a table containing most likely tercile and its associated confidence level (regarding probability value), with both coordinates (shape ready for shapefile conversion)
    ...
    for (t in 1:ndimT){
      ...
      for (i in 1:ngrids) {
        proba <- c(monthprob_ps[i,y_index[1],t,1],monthprob_ps[i,y_index[1],t,2],monthprob_ps[i,y_index[1],t,3]) # proba associated to each tercile 
        tercile <- which.is.max(proba) # most likely tercile
        matrix4shp[i,1] <- tercile 
        
        if (monthprob_ps[i,y_index[1],t,tercile] > 0.5){ # likely tercile confidence
        matrix4shp[i,2] <- 1
       }
   }
...
for (ind in 1:8){
  ...
  ## creating a table containing most likely tercile and its associated confidence level (regarding probability value), with both coordinates (shape ready for shapefile conversion)
      ...
      matrix4shp <- matrix(0, nrow = ngrids, ncol = 4)
      for (i in 1:ngrids) {
        proba <- c(seasprob_ps[i,y_index[1],1],seasprob_ps[i,y_index[1],2],seasprob_ps[i,y_index[1],3]) # proba associated to each tercile - warning: we're inverting to have lower tercile 1st
        tercile <- which.is.max(proba) # most likely tercile
        matrix4shp[i,1] <- tercile 
        
        if (seasprob_ps[i,y_index[1],tercile] > 0.5){ # likely tercile confidence
        matrix4shp[i,2] <- 1
       }
   }
