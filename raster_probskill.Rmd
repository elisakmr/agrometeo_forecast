...
for (ind in 1:8){
    ...
    wlt <- wilcox.test(score_ps[i,1,], conf.level = 0.95, na.action = na.omit)
    wht <- wilcox.test(score_ps[i,2,], conf.level = 0.95)
  ...
  mask.raster <- raster(resolution=c(0.1,0.072), crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0", ## !!!!!!!!!!!
                          ext = ext_prev)
  ras_prev_complete <- rasterize(shp_prob, mask.raster)
...
score_ps <- array(NA, dim=c(ngrids, ndimT, event, 1000)) 
  ...
  wlt <- wilcox.test(score_ps[i,t,1,], conf.level = 0.95)
  wht <- wilcox.test(score_ps[i,t,2,], conf.level = 0.95)
