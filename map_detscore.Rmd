...
for (m in 1:ndimT){
  ...
  maprev [[m]]<- 
    ...
    tm_fill(col = "MSESS", style="cont", breaks = seq(-3e-04, 3e-04, length.out = 5), 
...
com_legend <- qtm(shp_score[[m]], fill = "MSESS", fill.style="cont", attr.outside = TRUE, borders = NULL, fill.palette = score_palette,
                  fill.breaks = seq(-3e-04, 3e-04, length.out = 5))+ 
...
ggmap(back_map) +
  ...
  scale_fill_gradientn(colours=score_palette, limits = day25)+ 
