...
for (m in 1:ndimT){
  ...
  maprev [[m]]<- 
    ...
    tm_fill(col = "MSESS", style="fixed", breaks = c(-Inf, seq(-0.0003, 0.0001, length.out = 10), Inf), 
...
com_legend <- qtm(shp_score[[m]], fill = "MSESS", fill.style="fixed", attr.outside = TRUE, borders = NULL, fill.palette = score_palette,
                  fill.breaks=c(-Inf, seq(-0.0003, 0.0001, length.out = 10), Inf))+ 
...
ggmap(back_map) + 
  geom_sf(data = shp_msess, aes(fill = msess), inherit.aes = FALSE, alpha = 0.8)+
