  ...
  for (t in 1:ndimT){
    for (i in 1:ngrids){
      if (type == "obs") { int_df <- array_cont[i,,t] }
          ...
          hterc_vect <- binprob(int_df, nban=nanne, prob=1/3, rang="H", type)
          mterc_vect <- binprob(int_df, nban=nanne, prob=1/3, rang="M", type)
          lterc_vect <- binprob(int_df, nban=nanne, prob=1/3, rang="L", type)
          hquin_vect <- binprob(int_df, nban=nanne, prob=1/5, rang="H", type)
          mquin_vect <- binprob(int_df, nban=nanne, prob=1/5, rang="M", type)
          lquin_vect <- binprob(int_df, nban=nanne, prob=1/5, rang="L", type)
  
      array_binprob[i,,t,1:6] <- c(lterc_vect, mterc_vect, hterc_vect, lquin_vect, mquin_vect, hquin_vect) 
    }
  }
