...
score_det <- array(NA, dim=c(ngrids, 3, ndimT, nboot))
score_prps <- array(NA, dim=c(ngrids, ndimT, 2, nboot))
  ...
  for (b in 1:nboot)  {
      ...
      score_det[,,,b] <- det_prev[,,]

      nom_prev <- paste0(indic[ind], b,"scoredecilemonth_grid_prev.RData")
      load(file = file.path(dir_data, "score", nom_prev))

      score_prps[,,,b] <- scoreprob_month_ps[,,]

  }

  # getting na value indices (no score distinction: HT and LT mixed together, same as RMSE/ACC/R²)

int_det[[ind]]<-which(is.na(score_det), arr.ind = TRUE)
...
for (ind in 1:nindic){
  if (sum(which(df_error[,para,ind]>10))==0){
    histo[[ind]] <- ggplot() + geom_point() + labs(title = ind_eng[ind], subtitle = paste0("no monthly ",para))+
      theme(
...
for (ind in 1:nindic){
  ...
  score_det <- array(NA, dim=c(ngrids, 3, nboot))
