...
foreach(b=1:10, .packages = 'easyVerification') %dopar%  { # loop on bootstrap samples
  ...
  for (t in 1:ndimT){
    ...
    # 
     ...
     scoreprob_ps[,t,1] <- veriApply("EnsRoca", fcst=array_prev[,,,t], obs=array_obs[,,t], prob=1:2/3, parallel = TRUE)$cat3 
    ...
    #veriApply("EnsRoca", fcst=array_prev[19,,,t], obs=array_obs[19,,t], prob=1:4/5, parallel = TRUE)$cat5
      # CLIMATO
     scoreprob_clim[,t,1] <- veriApply("EnsRoca", fcst=array_clim[,,,t], obs=array_obs[,,t], prob=1:2/3, parallel = TRUE)$cat3 
