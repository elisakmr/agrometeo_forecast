...
foreach(b=1:nboot, .packages="tidyverse") %dopar%  { # loop on bootstrap samples
  ...
  if(ind==2|ind==3|ind==4|ind==5){
      prev_seas <- apply(array_prev[,,,month], c(1,2,3), sum) #### aggregation method to be changed!!
      ref_seas <- apply(array_obs[,,month], c(1,2), sum)
  } else {prev_seas <- apply(array_prev[,,,month], c(1,2,3), mean) 
  ref_seas <- apply(array_obs[,,month], c(1,2), mean)}
